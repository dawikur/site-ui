...
class Element {
	...
	getAttribute(name) { return this.attributes[name]; }
		...
		querySelector(selector) { return selector === ".ui-copy-feedback" ? this.children.find((child) => child.className === "ui-copy-feedback") : undefined; }
}
...
function runAsset(name, setup) {
	...
	const siteBack = new Element({ dataset: { siteBack: "" } });
	const siteBackSeparator = new Element({ dataset: { siteBackSeparator: "" } });
	...
	const document = {
		...
		querySelectorAll(selector) {
			...
			if (selector === "[data-site-back], [data-site-back-separator]") return [siteBack, siteBackSeparator];
			return [];
		},
	};
	const context = {
	...
	return { buttons, context, links, mediaListeners, root, siteBack, siteBackSeparator, windowListeners };
}
...
test("theme keeps unknown parameters while synchronising controls and transfer links", () => {
	...
	assert.equal(root.dataset.uiMotionReady, "true");
...
test("site back links are hidden only outside the assembled website", () => {
	const setup = { prefersDark: false, reducedMotion: false };
	const { context, siteBack, siteBackSeparator } = runAsset("theme.js", setup);
	assert.equal(siteBack.hidden, false);
	assert.equal(siteBackSeparator.hidden, false);
	context.window.location.protocol = "file:";
	context.window.SiteUi.url.updateSiteBackLinks();
	assert.equal(siteBack.hidden, true);
	assert.equal(siteBackSeparator.hidden, true);
});
...
test("owned parameters are updated together without dropping unknown parameters", () => {
	const setup = { prefersDark: false, reducedMotion: false };
	const { context, links } = runAsset("theme.js", setup);

	context.window.SiteUi.url.setOwnedParameters([
		{ name: "theme", value: "auto", defaultValue: "auto" },
		{ name: "contrast", value: "hard", defaultValue: "normal" },
	]);

	assert.equal(context.window.location.search, "?future=kept&contrast=hard");
	assert.equal(links[0].href, "https://example.test/next/?future=kept&contrast=hard");
});

test("copy feedback is shown only after a successful clipboard write", async () => {
	const writes = [];
	const setup = {
		prefersDark: false,
		reducedMotion: false,
	...
	const { context } = runAsset("theme.js", setup);
	const target = new Element();

	assert.equal(await context.window.SiteUi.copy.withFeedback(target, "#00bedc"), true);
	assert.deepEqual(writes, ["#00bedc"]);
	assert.equal(target.querySelector(".ui-copy-feedback").textContent, "Copied");
	assert.equal(target.querySelector(".ui-copy-feedback").getAttribute("role"), "status");
	assert.equal(target.querySelector(".ui-copy-feedback").getAttribute("aria-live"), "polite");
});
...
test("component contract keeps geometry and component motion centralised", () => {
	...
	for (const primitive of [".ui-control-dock", ".ui-segmented", ".ui-segmented-option", ".ui-tag", ".ui-action", ".ui-back-link", ".ui-pill", ".ui-toggle", ".ui-card", ".ui-code", ".ui-tooltip", ".ui-swatch", ".ui-palette-card", ".ui-scroll-top"]) assert.ok(css.includes(primitive));
	...
	assert.match(css, /\.ui-control-dock \{[^}]*position: fixed[^}]*top: var\(--space-2\)[^}]*z-index: 20/);
	assert.match(css, /\.ui-control-dock--grid-at-mobile \{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
	assert.doesNotMatch(css, /prefers-reduced-motion/);
	assert.match(foundation, /@media \(prefers-reduced-motion: reduce\)/);
	assert.match(foundation, /--ui-motion-duration: 260ms/);
	assert.match(foundation, /--ui-interaction-duration: 140ms/);
	assert.match(foundation, /transition-duration: 0\.01ms !important/);
	assert.match(foundation, /\.ui-visually-hidden \{[^}]*position: absolute[^}]*width: 1px[^}]*height: 1px/);
});
...
test("catalogue loads every shared asset and documents public component classes", () => {
	...
	assert.match(catalogue, /Successful feedback is announced politely/);
	for (const component of [
		"ui-control-dock", "ui-control-dock--grid-at-mobile", "ui-visually-hidden", "ui-segmented", "ui-segmented--two", "ui-segmented--vertical", "ui-segmented--vertical-at-wide", "ui-segmented--contrast", "ui-segmented-option",
		"ui-tag", "ui-tag--fill-at-mobile", "ui-pill", "ui-pill--color", "ui-pill--stacked-at-mobile", "ui-color-dot", "ui-toggle",
		"ui-action", "ui-action--landing", "ui-action--pending", "ui-back-link", "ui-card", "ui-card--landing", "ui-card--compact", "ui-card--flush", "ui-code", "ui-tooltip",
